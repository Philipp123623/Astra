    ...
    async def init_tables(self):
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS reactionrole_entries (id INT AUTO_INCREMENT PRIMARY KEY, message_id BIGINT NOT NULL, role_id BIGINT NOT NULL, label VARCHAR(100) NOT NULL, emoji VARCHAR(100), FOREIGN KEY (message_id) REFERENCES reactionrole_messages(message_id) ON DELETE CASCADE)")
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS reactionrole_messages (message_id BIGINT PRIMARY KEY, guild_id BIGINT NOT NULL, channel_id BIGINT NOT NULL, style VARCHAR(10) NOT NULL, embed_title VARCHAR(256) NOT NULL, embed_description TEXT NOT NULL, embed_color INT NOT NULL)")
                await cur.execute("CREATE TABLE IF NOT EXISTS reactionrole_entries (message_id BIGINT NOT NULL, role_id BIGINT NOT NULL, label VARCHAR(100) NOT NULL, emoji VARCHAR(100), FOREIGN KEY (message_id) REFERENCES reactionrole_messages(message_id) ON DELETE CASCADE)")
...
# Ollama-Config
OLLAMA_MODEL = "deepseek-coder:1.3b-instruct-q4_K_M"
OLLAMA_URL = "http://127.0.0.1:11434/api/generate"

# Cache für Antworten, damit wiederholte Tests nicht neu generieren
AI_CACHE = {}
AI_TTL = 6 * 3600  # 6 Stunden gültig
AI_TIMEOUT = 8     # Sekunden Timeout pro Anfrage (Normalfall)
AI_MAX_OUT = 800   # Maximale Zeichen
...
def local_ai_tips(
    ...
    """Fragt Ollama lokal nach kurzen Debug-Tipps."""
    ...
    prompt = (
        "Antworte ausschließlich mit genau 3 Bulletpoints.\n"
        "Format:\n"
        "- <Lösungsschritt 1>\n"
        "- <Lösungsschritt 2>\n"
        "- <Lösungsschritt 3>\n"
        ...
        "- Kein anderer Text vor oder nach den Bulletpoints.\n"
        "- Keine Erklärungen, Beispiele, Codeblöcke oder Markdown.\n"
        "- Keine Nummern, Überschriften oder Einleitung.\n"
        "- Keine Wiederholung der Aufgabe.\n"
        "- Wenn unsicher, erfinde plausible generische Fix-Schritte.\n\n"
        f"Fehler: {short_exc}\n"
        f"Ort: {origin}\n"
...
async def ollama_test(ctx, *, fehler: str = "ValueError: invalid literal for int() with base 10: 'abc'"):
    ...
    # Erster Versuch: Langer Timeout für Modell-Start
    ...
    # Falls erster Versuch fehlschlägt
    if not ai_tips:
        await ctx.send("⏳ Modell lädt noch… zweiter Versuch.")
        ai_tips = local_ai_tips(
            origin="test_command.py:42 in ollama_test()",
            code_line="int('abc')",
            short_exc=fehler,
