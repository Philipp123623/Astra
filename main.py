    ...
    async def init_tables(self):
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    "CREATE TABLE IF NOT EXISTS community_goals (id INT AUTO_INCREMENT PRIMARY KEY, guild_id BIGINT NOT NULL, started_at BIGINT NOT NULL, ends_at BIGINT NOT NULL, reward_role_id BIGINT NULL, reward_text VARCHAR(255) NULL, active BOOLEAN DEFAULT TRUE, channel_id BIGINT NOT NULL, msg_id BIGINT NOT NULL)")

                await cur.execute(
                    "CREATE TABLE IF NOT EXISTS community_goal_conditions (id INT AUTO_INCREMENT PRIMARY KEY, goal_id INT NOT NULL, type VARCHAR(32) NOT NULL, target BIGINT NOT NULL, progress BIGINT NOT NULL DEFAULT 0, FOREIGN KEY (goal_id) REFERENCES community_goals(id) ON DELETE CASCADE)")
