...
class Astra(commands.Bot):
    ...
    async def init_tables(self):
        ...
        async with self.pool.acquire() as conn:
            async with conn.cursor() as cur:
                # Tabellen erstellen
                await cur.execute("CREATE TABLE IF NOT EXISTS subscriptions (guild_id BIGINT NOT NULL, discord_channel_name VARCHAR(255) NOT NULL, platform ENUM('youtube','twitch') NOT NULL, content_id VARCHAR(255) NOT NULL, ping_role_id BIGINT NULL, PRIMARY KEY (guild_id, discord_channel_name, platform, content_id))")
                ...
                await cur.execute("CREATE TABLE IF NOT EXISTS backups (code VARCHAR(32) PRIMARY KEY, guild_id BIGINT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, version INT NOT NULL, size_bytes INT NOT NULL, `hash` BINARY(16) NOT NULL, `data_blob` LONGBLOB NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;")
