...
class InfoGroup(app_commands.Group):
    ...
    @app_commands.command(name="kanal")
    ...
    async def channelinfo(self, interaction: discord.Interaction, textchannel: discord.TextChannel = None,
        ...
        if isinstance(textchannel, discord.TextChannel):
            ...
            embed.add_field(name="🖌 Beschreibung",
                            value=f"{channel.topic if channel.topic else 'Keine Beschreibung.'}",
                            inline=False)
    ...
    @app_commands.command(name="rolle")
    @app_commands.checks.cooldown(1, 3, key=lambda i: (i.guild_id, i.user.id))
    async def roleinfo(self, interaction: discord.Interaction, role: discord.Role):
        ...
        em.add_field(name='👥 Personen in der Rolle',
                     value=f"{str(len(role.members))} von {interaction.guild.member_count} Mitgliedern.",
                     inline=True)
        await interaction.response.send_message(embed=em)

    @app_commands.command(name="wetter")
    @app_commands.checks.cooldown(1, 3, key=lambda i: (i.guild_id, i.user.id))
