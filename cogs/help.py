...
class HelpCog(commands.Cog):
    ...
    def _build_pages(self):
        self.pages = {
            ...
            "backups": f"> <:Astra_support:1141303923752325210> Sichere deinen Server mit einem Klick – Backups erstellen, wiederherstellen oder rückgängig machen. Immer sicher, immer unter Kontrolle.\n\n> **👥 × User Befehle:**\n> Keine User Befehle.\n\n> **👮‍♂ × Team Befehle:**\n> </backup create:{self._cid('backup create')}> - Erstellt ein Server-Backup.\n> </backup load:{self._cid('backup load')}> - Stellt ein Backup mithilfe eines Codes wieder her.\n> </backup undo:{self._cid('backup undo')}> - Setzt den Server auf den Stand vor der letzten Wiederherstellung zurück.\n> </backup status:{self._cid('backup status')}> - Zeigt den Fortschritt oder das Ergebnis des letzten Backup- oder Wiederherstellungsvorgangs.\n> </backup delete:{self._cid('backup delete')}> - Lösche ältere Backups.\n> </backup list:{self._cid('backup list')}> - Listet alle Backups dieses Servers mit Details auf."
        }
